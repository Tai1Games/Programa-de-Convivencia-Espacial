...
void PlayState::init() {
	...
	physicsWorld_ = new b2World(b2Vec2(0, -9.81));
	...
	//Entity* spaceJunk = entityManager_->addEntity();
																			// x, y, width, height,		density, friction, restitution, groupIndex, sensor
	Collider* collTinky = tinky->addComponent<Collider>(physicsWorld_, b2_dynamicBody, 0, 10, 1, 1,		5, 0.1, 1, 1, false);
	Collider* collSuelo = ground->addComponent<Collider>(physicsWorld_, b2_staticBody, 2, 2, 1, 1,	10, 0.2, 1, 0, false);
	...
	//collJunk->applyForce(b2Vec2(0, 4000000000), b2Vec2(300, 60));
